...
class SupperAuth
{
    ...
    {
        if(Redis::exists('auth_user_id'.\session('user')->us_id)){
            $routeList = collect(json_decode(Redis::get('auth_user_id'.\session('user')->us_id),true));
        }else {
            ...
            Redis::set('auth_user_id'.\session('user')->us_id,$routeList);
        }
