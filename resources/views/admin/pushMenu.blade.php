...
@section('content')
    ...
    <div class="result_wrap">
        ...
        <form method="post" action="{{url('admin/pushMenu/'.$info->us_id)}}">
            ...
            <table class="add_tab">
                ...
                <tr>
                    <th><i class="require">*</i>菜单选定：</th>
                    <td>
                        @foreach($list as $v)
                            <label><input name="menu_id[]" @if(in_array($v->id,explode(',',isset($info->menu_id)?$info->menu_id:''))) checked="checked" @endif type="checkbox" value="{{$v->id}}">{{$v->name}}</label>
                        @endforeach
                    </td>
                </tr>
                <tr>
